...
def formVector(data, lablelist):
    x_train = [np.hstack((model.sv(x[kTEXT_FIELD]), model.sv(x["answer" + i])))
...
if __name__ == "__main__":
    ...
    x_train, y_train = formVector(train, lablelist)
    # print("new total x_train length", x_train.shape[0])

    x_test, y_test = formVector(test, lablelist)
